...
#
# —[ GENERAL ]— CONVERT HEX COLOR VALUE
# Converts a hex value value to a (nearest) 0-256 color index
#
#   @param {string} [$1] Defined hex color value
#
function fromhex() {
    hex=${1#"#"}
...
function getkeycloaktoken() {
    if [ -z "${1}" ]; then
        ...
        source $DEV/mbo/venvs/local-start/bin/activate;
        echo -e "${bold}${fgYellow}Generating authenication token for ${KC_USERNAME} ... ${reset}";
        python $DEV/mbo/scripts/local_start/local_start.py -a connect-webapp -u ${KC_USERNAME} -p ${KC_PASSWORD} -v $ROLE_TYPE | pbcopy;
        deactivate;
        echo
